...
MyKeymapMenuExit() {
  global IsDesktopHostMode

  if IsDesktopHostMode {
    try ProcessClose("MyKeymapDesktop.exe")
    return
  }
 ...
 */
MyKeymapOpenSettings() {
  if FileExist("./MyKeymapDesktop.exe") {
    if WinExist("MyKeymap-re Desktop") {
      WinActivate("MyKeymap-re Desktop")
      return
    }
    Run("./MyKeymapDesktop.exe", A_WorkingDir)
    return
  }
